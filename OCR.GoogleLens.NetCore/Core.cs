...
public class Core
{
    ...
    public async Task<List<string>> ScanByBufferAsync(byte[] buffer)
    {
        ...
        if (format.DefaultMimeType != null && !format.FileExtensions.Any())
        {
        ...
        return await ScanByData(buffer, format.DefaultMimeType, [Width, Height]);
    }
